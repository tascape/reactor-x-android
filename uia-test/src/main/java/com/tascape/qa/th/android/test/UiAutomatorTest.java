 ...
 */
package com.tascape.qa.th.android.test;
...
public interface UiAutomatorTest {
    ...
    default void testManually(UiAutomatorDevice device) throws Exception {
     ...
     * with the app-under-test manually.
     ...
     */
    default void testManually(UiAutomatorDevice device, int timeoutMinutes) throws Exception {
        final Logger LOG = LoggerFactory.getLogger(UiAutomatorTest.class);

        LOG.info("Start UI to test manually");
        String info = device.getProductDetail();
                    ...
                    Thread t = new Thread(tName) {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                File f = File.createTempFile("dump-", ".xml");
                                device.dumpWindowHierarchy(f.getAbsolutePath());
                                IOUtils.readLines(new FileInputStream(f)).forEach(line -> {
                                    LOG.debug(line);
